...
def add_events(draw_black, events, event_counts, event_font, cal_pos, day_w, event_h, title_h, holidays = False):
    cal_x, cal_y = cal_pos
...
def draw_calendar(image_black, image_red, events, holidays):
    ...
    draw_black.text((10, 10), '{} {} {} {}'.format(weekdays[now.weekday()], now.day, months[now.month-1], now.year), font=big_font)
    ...
    event_counts = [0]*7
    event_counts = add_events(draw_black, holidays, event_counts, event_font, (cal_x, cal_y), day_w, event_h, title_h,
                              holidays=True)
    event_counts = add_events(draw_black, events, event_counts, event_font, (cal_x, cal_y), day_w, event_h, title_h)
